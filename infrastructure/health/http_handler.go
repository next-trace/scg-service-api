...
import (
	...
	apphealth "github.com/hbttundar/scg-service-base/application/health"
...
func (h *httpHandler) handleChecks(w http.ResponseWriter, r *http.Request, checkType apphealth.CheckType) {
	...
	if overallStatus == apphealth.StatusDown {
		statusCode = http.StatusServiceUnavailable
	} else if overallStatus == apphealth.StatusDegraded {
...
func RegisterHTTPHandlers(handler apphealth.Handler, mux *http.ServeMux, config apphealth.Config) {
	...
	// Register handlers
	mux.Handle(config.LivenessPath, handler.LivenessHandler().(http.Handler))
	mux.Handle(config.ReadinessPath, handler.ReadinessHandler().(http.Handler))
	mux.Handle(config.Path, handler.HealthHandler().(http.Handler))
}